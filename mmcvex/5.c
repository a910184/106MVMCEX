...
int main(void)
{
	...
	while(1)
	{
		...
		{
			case 'a':obyte= 200; break;
			case 'b':obyte= 201; break;
			case 'c':obyte= 202; break;
			case 'e':obyte= 204; break;
			case 'f':obyte= 205; break;//3:0
		}
		switch (I_PORT)
		{
			case 'a':ibyte=200; break;
			case 'b':ibyte=201; break;
			case 'c':ibyte=202; break;
			case 'e':ibyte=204; break;
			case 'f':ibyte=205; break; //3:0
		}
		switch (I_bit)//MASK
		{
		...
		while(1)
		{
			...
			check1=M128_DIO_fgt(gbyte,gmask,gshift,&data);
			check2=M128_DIO_fpt(pbyte,pmask,pshift,&data);
			printf("get:%d , put:%d\n",check1,check2);
			printf("data %d\n",data);
			if(time==100)
			{
